...
// Format date helper (omitted for brevity)
const formatDate = (dateString) => {
    if (!dateString) return 'N/A';
    const date = new Date(dateString);
    return date.toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' });
...
// Format number with 2 decimals (omitted for brevity)
const formatNumber = (num) => {
    const number = parseFloat(num);
    if (isNaN(number)) return '0.00';
    return number.toFixed(2);
...
// KPI Card Component (omitted for brevity)
const KPICard = ({ title, value, subtitle, icon: Icon, loading, color }) => (
// ... (KPICard component body)
    <div className="bg-white p-6 rounded-2xl shadow-lg hover:shadow-xl transition-shadow">
        <div className="flex items-center justify-between mb-4">
            <h3 className="text-xs font-medium tracking-wide uppercase" style={{ color: '#666' }}>
                {title}
...
// Expandable Row Component for Harvests (omitted for brevity)
const ExpandableHarvestRow = ({ harvest, isExpanded, onToggle, blockDetails }) => {
// ... (ExpandableHarvestRow component body)
    return (
        <>
            <tr className="border-b border-gray-100 transition-colors duration-150 hover:bg-light-coffee-brown/40">
                <td className="px-6 py-3 text-left font-medium text-gray-800 text-sm">{harvest.harvest_id || 'N/A'}</td>
...
                                    <div>
                                        <p className="text-xs font-semibold text-gray-500 uppercase">Coffee Type</p>
                                        <p className="text-sm text-gray-800">{harvest.coffee_type}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs font-semibold text-gray-500 uppercase">Weight on Delivery</p>
                                        <p className="text-sm text-gray-800 font-semibold">{formatNumber(harvest.weight_on_delivery)} kg</p>
...
    // Fetch harvests (CORRECTED)
    const fetchHarvests = useCallback(async () => {
        setLoading(true);
        setError(null);
        try {
            console.log('Fetching harvests from:', HARVESTS_API);
            const headers = getAuthHeaders(); // Use the helper
            
            const response = await fetch(HARVESTS_API, { headers }); // Pass headers here
            
            console.log('Response status:', response.status);
            console.log('Response OK:', response.ok);
...
    // Fetch blocks (CORRECTED - **This was the main source of the 401 error**)
    const fetchBlocks = useCallback(async () => {
        try {
            console.log('Fetching blocks from:', BLOCKS_API);
            
            const headers = getAuthHeaders(); // Use the helper
            
            // Pass the headers with the Authorization token here
            const response = await fetch(BLOCKS_API, { headers }); 
            console.log('Blocks response status:', response.status);
...
                console.error('Failed to fetch blocks. Status:', response.status);
                console.error('Error response:', errorText);
                // The blocks data is crucial for the table, log an error but don't stop the app
            }
        } catch (error) {
            console.error('Error fetching blocks:', error);
        }
...
    useEffect(() => {
        // NOTE: This assumes 'authToken' is set correctly in localStorage after a successful login 
        // in your 'Login.jsx' component.
        fetchHarvests();
        fetchBlocks();
    }, [fetchHarvests, fetchBlocks]);

    // ... (Rest of the component logic for filtering, rendering, etc.)

    // Toggle row expansion (omitted for brevity)
    const toggleRow = (harvestId) => {
        setExpandedRows(prev => ({
            ...prev,
            [harvestId]: !prev[harvestId]
...
    // Get unique coffee types for filter (omitted for brevity)
    const uniqueBlocks = [...new Set(harvests.map(h => h.coffee_type || h.block_id).filter(Boolean))];

    // Filter harvests (omitted for brevity)
    const filteredHarvests = harvests.filter(harvest => {
        const matchesSearch =
            !searchTerm ||
            (harvest.name && harvest.name.toLowerCase().includes(searchTerm.toLowerCase())) ||
...
                {/* Error Alert (omitted for brevity) */}
                {error && (
                    <div className="mb-4 p-4 bg-red-50 border-l-4 border-red-500 rounded-lg">
                        <div className="flex items-center">
                            <div className="flex-shrink-0">
...
                {/* Action Bar & Filter (omitted for brevity) */}
                <div className="mb-4 flex flex-wrap justify-between items-center gap-3">
                    <div className="flex gap-3 items-center w-full sm:w-auto order-2 sm:order-1">
                        <div className="relative flex-grow">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
...
                {/* Harvests Table (omitted for brevity) */}
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead style={{ backgroundColor: CoffeeColors.ACTIVE_LINK_BG }}>
