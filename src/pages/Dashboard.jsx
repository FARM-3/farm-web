...
const SidebarLink = ({ icon: IconComponent, title, href, isActive, isCollapsed }) => {
    ...
    return (
        ...
        >
            ...
            />
            <span
                className={`whitespace-nowrap transition-opacity duration-200 ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100 block'}`}
            >
                {title}
            </span>
        </a>
...
const App = () => {
    ...
    const MainSidebar = () => (
        <>
            ...
            <aside
                className={`bg-accent-btn shadow-xl h-full fixed top-0 left-0 z-50 p-4 flex flex-col
            ...
            >
                ...
                <div className="h-16 flex items-center justify-between px-2 mb-6">
                    ...
                    >
                        ...
                        {isSidebarOpen ? (
                             <XIcon className="w-6 h-6 text-gray-600" strokeWidth={2.2} />
                        ) : (
                            <MenuIcon className="w-6 h-6 text-gray-600" strokeWidth={2.2} />
                        )}
                    </button>
                </div>
                ...
                <nav className="flex-1 space-y-2">
                    <SidebarLink icon={DashboardIcon} title="Dashboard" isActive={true} isCollapsed={isTextHidden}/>
    ...
    const MainHeader = () => (
        <header className="bg-accent-header rounded-xl p-4 flex items-center justify-between shadow-md mb-8 z-30 relative">
            <div className="flex items-center">
                {/* Mobile Menu Button (always visible on mobile to open sidebar) */}
                <button 
                    className="p-2 mr-3 rounded-full md:hidden hover:bg-accent-header/50" 
                    onClick={toggleSidebar}
                >
                    <MenuIcon className="w-6 h-6 text-gray-700" strokeWidth={2.2} />
                </button>
                
                {/* Desktop Collapse Button (only visible on desktop when sidebar is w-16) - Now handled in MainSidebar for better placement */}
                <h2 className="text-xl md:text-2xl font-bold text-gray-800">Financial Dashboard</h2>
            </div>
            
            <div className="flex items-center space-x-4">
                <button className="p-2 rounded-full hover:bg-accent-header/50">
                    <BellIcon className="w-6 h-6 text-gray-700" strokeWidth={2.2} />
                </button>
                <div className="w-10 h-10 rounded-full bg-accent-btn/50 flex items-center justify-center text-sm font-bold text-white">
                    JP
                </div>
            </div>
        </header>
