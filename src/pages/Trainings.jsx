...
const API = `${import.meta.env.VITE_API_URL}/api/config/trainings/`;

const empty = { title: '', topic: '', trainer: '', location: '', start_date: new Date().toISOString().slice(0, 10), end_date: '', attendees: 0, status: 'scheduled', notes: '' };
      ...
      <main className="p-6">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Extension sessions for farmers and staff</p>
          </div>
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50"><tr className="text-left text-gray-500">
              ...
              <th className="px-4 py-3">Trainer</th><th className="px-4 py-3">Attendees</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 w-12"></th>
            </tr></thead>
            <tbody>
              {records.map(r => (
                <tr key={r.id} className="border-t">
                  ...
                  <td className="px-4 py-3"><button onClick={() => remove(r.id)} className="text-red-500"><Trash2 size={16} /></button></td>
                </tr>
              ))}
              {!records.length && <tr><td colSpan={7} className="px-4 py-10 text-center text-gray-400">No trainings scheduled</td></tr>}
            </tbody>
          </table>
        </div>
