...
const Hulling = () => {
    ...
    const fetchRecords = useCallback(async () => {
        ...
        try {
            ...
            const mockData = [
                { id: 1, harvest_id: 'H-2024-001', input_weight: '3000kg', output_weight: '2400kg', efficiency: '80%', date: '2024-01-18' },
                { id: 2, harvest_id: 'H-2024-002', input_weight: '2100kg', output_weight: '1680kg', efficiency: '80%', date: '2024-01-17' },
            ];
            setRecords(mockData);
        } catch (error) {
    ...
    const avgEfficiency = records.length > 0
        ? (records.reduce((sum, r) => sum + parseFloat(r.efficiency), 0) / records.length).toFixed(1)
        : '0';
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead style={{ backgroundColor: CoffeeColors.LIGHT_BG }}>
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-center text-xs font-semibold uppercase text-gray-700">Input Weight</th>
                                    <th className="px-6 py-3 text-center text-xs font-semibold uppercase text-gray-700">Output Weight</th>
                                    <th className="px-6 py-3 text-center text-xs font-semibold uppercase text-gray-700">Efficiency</th>
                            ...
                            <tbody className="bg-white divide-y divide-gray-100">
                                {loading ? (
                                    ...
                                    <tr>
                                        <td colSpan="6" className="px-6 py-12 text-center text-gray-500">No hulling records found</td>
                                    </tr>
                                ) : (
                                    records.map((record) => (
                                        <tr key={record.id} className="hover:bg-gray-50">
                                            <td className="px-6 py-4 font-medium text-gray-800">{record.harvest_id}</td>
                                            <td className="px-6 py-4 text-center text-gray-700">{record.input_weight}</td>
                                            <td className="px-6 py-4 text-center text-gray-700">{record.output_weight}</td>
