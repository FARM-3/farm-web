...
import { RefreshCw, DollarSign, Calendar, User, MinusCircle, Wallet, Loader2, ArrowUp, ArrowDown } from 'lucide-react'; 
...
const Wage_API_Endpoint = 'http://127.0.0.1:8000/api/wages/';
    ...
    const renderTableContent = () => {
        ...
        return sortedWages.map((wage, index) => (
            <tr key={index} className="border-b transition-colors duration-150 hover:bg-white/50">
                ...
                <td className="px-6 py-3 text-right text-gray-800 font-semibold">{formatCurrency(wage.monthly_pay)}</td>
                <td className="px-6 py-3 text-right text-green-700 font-bold">{formatCurrency(wage.amount_paid)}</td>
                <td className="px-6 py-3 text-right text-red-600">{formatCurrency(wage.deduction)}</td>
